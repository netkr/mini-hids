...
Mini-HIDS 是一个基于 Python 原生库的零依赖、智能化 Linux 服务器防御工具。它采用 C/S（客户端/服务器）架构，实现对暴力破解与 Webshell 的分钟级处置能力。

**架构概览**
- **mini_hids.py**（数据面 / 后台守护进程）：7×24 小时运行，负责底层监控与自动防御
- **hids_cli.py**（控制面 / Agent 专属调用接口）：命令行工具，供 Agent 调用，执行完立刻返回标准 JSON 格式字符串

**安装方法**
1、**传统部署**：通过 git 部署到云服务器，运行守护进程并使用 CLI 工具进行管理
2、**Agent 集成**：把本项目链接发给 agent（比如 openclaw、hermes agent），并告诉它“把这个项目封装成 skill”并授权即可；
...
2. **修改系统配置**
   编辑 `mini_hids.py` 文件，根据您的服务器环境调整配置：
   ```python
   CONFIG = {
       "LOG_PATHS": {
